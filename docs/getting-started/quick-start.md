...
| Value | Meaning |
|---|---|
| `auto` | **Current default.** Use the host-specific master eggNOG/GO GMT generated by `Create_custom_host.sh`, then filter it against `host.gct` for the current run |
| `default` | Explicitly use the legacy MSigDB C2 symbols GMT |
| `FILE` | Use a user-provided GMT file |

If `--ssgsea-gmt` is omitted, MTD Explorer uses `auto`. Use `--ssgsea-gmt default` only when you intentionally want the legacy MSigDB C2 behavior. The deprecated `custom_eggnog` value is retained only as a backward-compatible alias for `auto`.
...
The standard commands above run locally. On a prepared Slurm cluster, add an HPC configuration file:
...
bash ~/MTD-Explorer/MTD_explorer.sh \
  ...
  --hostid 9606 \
  --hpc-conf ~/MTD-Explorer/Installation/HPC/MTD_hpc_slurm.conf
```

With `--hpc-conf`, fastp, Kraken2 host and microbiome classification, Bracken, HUMAnN/MetaPhlAn, and optional Magic-BLAST can be distributed through Slurm. Omitting the option preserves local execution.

Before the first HPC run, install and validate the node-local runtime. See [HPC / Slurm execution](../user-guide/hpc-slurm.md).
...
MTD Explorer prints stage progress and sample-level progress messages, for example ` [Kraken2] [3/15] Processing SAMPLE `, so the number of completed and remaining samples is visible during sample-wise stages.

The run also records the exact input samplesheet and the resolved FASTQ files under `methods/`. HPC runs additionally create operational Slurm records under `hpc/`.
...
```bash
bash ~/MTD-Explorer/MTD_check_installation.sh \
  --mode full \
...
information about input files, analysis modes, custom hosts, optional HPC execution, and output
interpretation.
